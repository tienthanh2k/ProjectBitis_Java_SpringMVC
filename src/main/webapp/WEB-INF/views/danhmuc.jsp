				...
				<nav class="navbar navbar-default col-md-9" role="navigation">
					<div class="container-fluid">
						<div class="row">
							...
							<c:set var="maDanhMucDuocChon" scope="session"
								value="${listSanPhams.get(0).getDanhMuc().getMaDanhMuc()}" />
							<div class="collapse navbar-collapse col-md-9"
								...
								<ul class="nav navbar-nav">
									...
									<c:forEach var="item" items="${listDanhMucs}">
										<c:if test="${maDanhMucDuocChon == item.maDanhMuc}">  
										   <li ><a style="border: thin solid gray;" maDanhMuc="${item.maDanhMuc}" href="<c:url value='/danhmuc/${item.maDanhMuc}' />" >${item.tenDanhMuc}</a></li>
										</c:if> 
										<c:if test="${maDanhMucDuocChon != item.maDanhMuc}">  
										   <li><a maDanhMuc="${item.maDanhMuc}" href="<c:url value='/danhmuc/${item.maDanhMuc}' />" >${item.tenDanhMuc}</a></li>
										</c:if>   
		...
		<div class="row">
			<c:forEach var="item" items="${listSanPhams}">
				<div class="col-md-3 col-sm-12 text-center thongtinsanpham" id="${item.maSanPham}">
					...
					<p>
						<a class="item_add" href="<c:url value='/sanpham/${item.maSanPham}' />"><i></i> <span class="item_price">${item.donGia}</span></a>
					</p>
				</div>
			</c:forEach>
		</div>
